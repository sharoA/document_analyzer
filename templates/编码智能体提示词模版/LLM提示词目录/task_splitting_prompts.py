...
# 🔧 提示词工具类
class TaskSplittingPromptBuilder:
    """任务拆分提示词构建器"""
    
    @staticmethod
    def build_requirements_analysis_prompt(requirements_doc: str, project_name: str) -> str:
        """构建需求分析提示词"""
        return REQUIREMENTS_ANALYSIS_PROMPT.format(
            requirements_doc=requirements_doc,
            project_name=project_name
        )
    
    @staticmethod
    def build_design_analysis_prompt(design_doc: str, requirements_analysis: str) -> str:
        """构建设计分析提示词"""
        return DESIGN_ANALYSIS_PROMPT.format(
            design_doc=design_doc,
            requirements_analysis=requirements_analysis
        )
    
    @staticmethod
    def build_service_boundary_prompt(requirements_analysis: str, design_analysis: str) -> str:
        """构建服务边界识别提示词"""
        return SERVICE_BOUNDARY_PROMPT.format(
            requirements_analysis=requirements_analysis,
            design_analysis=design_analysis
        )
    
    @staticmethod
    def build_dependency_analysis_prompt(service_identification: str, design_doc: str) -> str:
        """构建依赖分析提示词"""
        return DEPENDENCY_ANALYSIS_PROMPT.format(
            service_identification=service_identification,
            design_doc=design_doc
        )
    
    @staticmethod
    def build_task_scheduling_prompt(service_identification: str, dependency_analysis: str) -> str:
        """构建任务调度提示词"""
        return TASK_SCHEDULING_PROMPT.format(
            service_identification=service_identification,
