...
# 创建SocketIO实例 - 优化实时性，禁用缓冲
socketio = SocketIO(
    app,
    cors_allowed_origins=["http://localhost:3000", "http://127.0.0.1:3000"],
    async_mode='eventlet',
    logger=False,  # 关闭详细日志避免性能影响
    engineio_logger=False,  # 关闭引擎日志
    max_http_buffer_size=1 * 1024 * 1024,  # 减少缓冲区：1MB
    ping_timeout=10,  # 大幅减少ping超时：10秒
    ping_interval=5,  # 大幅减少ping间隔：5秒
    # 🔥 关键：禁用各种缓冲优化，提升实时性
    compression=False,  # 禁用压缩避免延迟
    always_connect=True,  # 始终连接
    transports=['websocket']  # 强制websocket传输
)
...
def handle_get_analysis_progress(data):
    ...
    try:
        ...
        logger.info(f"📋 [进度查询] 客户端 {client_id} 请求任务 {task_id} 的进度")
        logger.info(f"📋 [进度查询] 当前活跃任务: {list(analysis_tasks.keys())}")
        logger.info(f"📋 [进度查询] 任务客户端映射: {task_clients}")
        
        if task_id in analysis_tasks:
            progress_manager = analysis_tasks[task_id]
            progress_data = progress_manager.to_dict()
            logger.info(f"📊 [进度查询] 找到任务，当前进度: {progress_data['overall_progress']}%")
            
            # 更新任务客户端映射
            task_clients[task_id] = client_id
            logger.info(f"🔄 [进度查询] 更新任务映射: {task_id} -> {client_id}")
            
            emit('analysis_progress', progress_data)
            logger.info(f"✅ [进度查询] 进度数据已发送给客户端 {client_id}")
        else:
            logger.error(f"❌ [进度查询] 任务 {task_id} 不存在")
            emit('analysis_error', {
                'task_id': task_id,
                'error': f'分析任务不存在。当前活跃任务: {list(analysis_tasks.keys())}',
                'timestamp': datetime.now().isoformat(),
                'available_tasks': list(analysis_tasks.keys())
            })
...
def run_automatic_analysis(task_id: str, input_data: Dict[str, Any]):
    ...
    try:
        ...
        if client_id:
            ...
            logger.info(f"🔧 [调试] 文档解析阶段完成 - 广播analysis_progress")
            socketio.emit('analysis_progress', progress_manager.to_dict())
        ...
        if client_id:
            ...
            logger.info(f"🔧 [调试] 内容分析阶段开始 - 广播analysis_progress")
            progress_data = progress_manager.to_dict()
            logger.info(f"📊 [调试] 即将发送的进度数据: {progress_data}")
            socketio.emit('analysis_progress', progress_data)
            logger.info(f"✅ analysis_progress已发送到所有客户端")
            
            # 🔧 应急：同时发送到特定客户端房间
            logger.info(f"🔧 [应急] 同时发送到客户端房间: {client_id}")
            socketio.emit('analysis_progress', progress_data, room=client_id)
        ...
        if client_id:
            ...
            logger.info(f"🔧 [调试] 内容分析阶段完成 - 广播analysis_progress")
            socketio.emit('analysis_progress', progress_manager.to_dict())
        ...
        client_id = validate_and_update_client_mapping(task_id)
        if client_id:
            logger.info(f"🔧 [调试] AI智能分析阶段开始 - 广播analysis_progress")
            socketio.emit('analysis_progress', progress_manager.to_dict())
        ...
        if client_id:
            ...
            logger.info(f"🔧 [调试] AI智能分析阶段完成 - 广播analysis_progress")
            socketio.emit('analysis_progress', progress_manager.to_dict())
            socketio.emit('analysis_completed', {
                'task_id': task_id,
