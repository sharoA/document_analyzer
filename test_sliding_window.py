...
from src.corder_integration.langgraph.nodes.task_splitting_node import SlidingWindowManager
...
def test_sliding_window():
    ...
    # 初始化窗口管理器
    window_manager = SlidingWindowManager(max_window_size=100, overlap_size=20)
    
    # 创建测试文档
    test_doc = """设计文档 - 一局对接链数优化V0.1
1. 系统架构设计
1.1 项目介绍
...
2.1.2 API设计：
2.1.2.1 新增接口：
GET /queryCompanyUnitList    #查询企业组织单元列表
...
2.2.2 API设计：
2.2.2.1 新增接口：
GET /crcl-open-api/lsLimit/listUnitLimitByCompanyId    #组织单元额度列表
...
本次新增接口，已经按服务范围进行划分，详见设计文档2服务设计部分。"""
    ...
    # 测试窗口分割
    windows = window_manager.split_text_into_windows(test_doc)
    
    print(f"📊 分割结果: {len(windows)} 个窗口")
    for i, window in enumerate(windows):
        print(f"窗口 {window['window_id']}: 位置 {window['start_pos']}-{window['end_pos']}, 长度 {len(window['content'])}")
        print(f"  内容预览: {window['content'][:50]}...")
        print()
    
    # 测试上下文管理
    print("🔄 测试上下文管理...")
    window_manager.add_context("设计分析完成，识别了微服务架构", "设计分析")
    window_manager.add_context("技术栈选择合理，使用主流技术", "技术栈分析")
    window_manager.add_context("部署架构完善，支持容器化", "部署分析")
    ...
    mock_results = [
        {
            "identified_services": [
                {"name": "用户服务", "description": "用户管理"},
                {"name": "订单服务", "description": "订单处理"}
            ],
            "summary": "识别了用户和订单服务"
        },
        {
            "identified_services": [
                {"name": "支付服务", "description": "支付处理"},
                {"name": "用户服务", "description": "重复的用户服务"}  # 测试去重
            ],
            "summary": "识别了支付服务"
        }
    ]
    
    merged_result = window_manager.merge_analysis_results(mock_results, "任务拆分")
